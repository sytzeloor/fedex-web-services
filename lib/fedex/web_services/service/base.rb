    ...
    class Base
      ...
      private
        def issue_request(request)
          ...
          if root_exception.respond_to?(:detail)
            Rails.logger.debug(root_exception.detail.fault.details.inspect)
            Rails.logger.debug(root_exception.detail.fault.details.to_s)
          end
